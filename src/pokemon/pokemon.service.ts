...
export class PokemonService {
  ...
  async findOne(term: string) {
    ...
    if (!pokemon && isValidObjectId(term)) {
      pokemon = await this.pokemonModule.findOne({ _id: term });
    }
    ...
    if (!pokemon) {
      throw new NotFoundException(
        `Pokemon not found with id, name or no "${term}" not found`,
      );
    }
  ...
  async remove(id: string) {
    // await this.pokemonModule.findByIdAndDelete(id);
    const { deletedCount } = await this.pokemonModule.deleteOne({
      _id: id,
    });
    if (deletedCount === 0) {
